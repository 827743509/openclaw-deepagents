"""ingestion_tasks 表访问层。

不负责 commit；service 层 / pipeline 控制事务边界。
"""
...
from ssw.db.models import IngestionTaskType, IngestionTask, IngestionTaskStatus
...
class IngestionTaskRepository:
    ...
    async def get_latest_by_document(
        ...
        """详情页右侧"最近一次任务"卡片用：取 created_at 最新一条。"""
        stmt = (
